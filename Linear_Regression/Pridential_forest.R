...
factorVariables <- c("Product_Info_1", "Product_Info_2", "Product_Info_3", "Product_Info_5", "Product_Info_6", "Product_Info_7", 
                     ...
                     "Medical_History_6", "Medical_History_7", "Medical_History_8", "Medical_History_9", "Medical_History_10", 
...
for (n in colnames(train[factorVariables])){
  train[[n]] <- as.factor(as.numeric(train[[n]]))
...
s <- sample(nrow(train), 0.75*nrow(train))
train_local <- train[s, ]
test_local <- train[-s, ]
...
target = 'Response'
features = colnames(train)[!(colnames(train) %in% c('Id', 'Response', 'Product_Info_2'))]

# Open H2o instance and convert data
h2o.init(nthreads = -1, max_mem_size = '4G', assertion = FALSE)
...
# Train model
set.seed(42)
rf <- h2o.randomForest(x = features, y = target,
                       ntrees = 150,
                       max_depth = 50,
                       training_frame = trainHex)

# measure accuracy
preds <- as.data.frame(h2o.predict(rf, testHex))
head(preds[,1], 10)

print("Creating submission file")
submit <- data.frame(Id = test$Id, Response = preds[,1])
write.csv(submit, "script_test.csv", row.names = FALSE)
