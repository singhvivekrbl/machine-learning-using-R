...
head(data)
```
##see whether there is any missing data
```{r}
...
summary(data)
```
##plot the amount against the class of transaction
```{r}
...
##To get a fuller picture, let's compute the mean and median values for each class.
```{r}
...
```
##apply that transformation to the "Amount" column 
```{r}
